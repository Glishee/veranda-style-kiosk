...
export default function CategoryStep({ categories, onPreview }: Props) {
  ...
  function handleNext(slug?: string) {
    const selectedSlug = slug ?? previewSlug
    if (!selectedSlug) return

    resetTimer()
    dispatch({ type: 'SET_CATEGORY', slug: selectedSlug })
  }
        ...
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`flex items-center justify-between w-full border-[1.5px] px-3.5 py-3 text-left text-[10px] font-bold tracking-[1.5px] uppercase transition-all duration-150 active:scale-[0.98] active:opacity-75 min-h-[48px] ${isSelected
